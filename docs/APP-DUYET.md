...
| app → máy | `query` | `what` (`status` · `so` · `brain` · `brain-tim` (+`tu`) · `brain-graph`) — hỏi, chỉ đọc, không cần chữ ký duyệt; máy trả `state` cùng `what`, riêng `brain-graph` là `{nodes:[{id,ten,loai,duong,mo_ta,so_link}], edges:[{a,b}]}` |
