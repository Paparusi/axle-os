...
| app → máy | `query` | `what` (`status` · `so` · `brain` · `brain-tim` (+`tu`) · `brain-graph` · `brain-moc` · `may-bao` · `tep` · `tep-mo`) — hỏi, chỉ đọc, không cần chữ ký duyệt; máy trả `state` cùng `what`, riêng `brain-graph` là `{nodes:[{id,ten,loai,duong,mo_ta,so_link}], edges:[{a,b}]}` |
...
| app → trạm | `POST /v1/blob` `{to, blob}` | tệp đính kèm (ảnh đã nén, Excel/Word/PDF nguyên bản ≤12MB) niêm phong cho máy bằng hộp X25519+ChaCha20 như tin → `{id}`; trạm giữ 1 giờ trong bộ nhớ, ≤16MB thân, ≤16 tệp chờ/người nhận; chỉ **người nhận** `GET /v1/blob/<id>` được, lấy một lần là xoá. Tin `hoi` mang `tep: [{id, ten}…]` (≤6) trong hộp đã ký (`anh: [id]` = bản app cũ). Máy lưu `~/.cache/axle-hoi/`, đổi Excel→CSV từng sheet, Word→txt, PowerPoint→PDF, PDF→txt (`core/desktop/doi-tep.sh`, LibreOffice/pdftotext dưới tài khoản chủ). Thử: `build/relay-blob-smoke.mjs` |
...
| app → máy | `query` `tep` | `duong` (tương đối so với nhà chủ, `""` = nhà), `xep` (`ten` · `moi` = sửa gần nhất trước) — xem thư mục trên máy (từ 0.1.160), chỉ đọc, không chữ ký duyệt. Máy trả `state tep` `{duong, hoi, ten, cha, ds:[{ten, thu_muc, co, sua}], bot, xep}` hoặc `{duong, hoi, ds:[], loi}`; `hoi` = đường app hỏi (lối tắt thì `duong` là chỗ thật). Cắt ở 400 mục và ~40 KB cho lọt hộp thư 80 KB, `bot` = số mục không gửi |
| app → máy | `query` `tep-mo` | `duong` — xin một tệp. Máy (`approve/tep.js`) chỉ gửi tệp trong nhà chủ: không chỗ ẩn (`.ssh`, `.config`…), không `~/snap` (hồ sơ trình duyệt snap), không tệp khoá/mật khẩu (`.pem`, `id_*`, `Cookies`, `Login Data`…), không tệp chủ máy tự không đọc được (bộ duyệt chạy root nên tự kiểm quyền như chủ), lối tắt ra ngoài / vào chỗ cấm bị chặn, ≤12 MB. Máy niêm phong tệp cho **đúng điện thoại hỏi** rồi `POST /v1/blob` (chiều máy → điện thoại; điện thoại đã `/v1/link` máy lúc ghép) → trả `state tep-mo` `{duong, hoi, ten, co, blob}` hoặc `{duong, hoi, loi}`. App `GET /v1/blob/<id>`, mở hộp, ghi thư mục tạm, mở bằng Xem nhanh (QuickLook); đóng là xoá. Nhật ký bộ duyệt ghi `mở tệp` (máy, đường, cỡ). Thử: `approve/test-tep.mjs`, `build/relay-blob-smoke.mjs` |
