...
| app → trạm | `POST /v1/blob` `{to, blob}` | tệp đính kèm (ảnh JPEG ≤ ~900KB, niêm phong cho máy bằng hộp X25519+ChaCha20 như tin) → `{id}`; trạm giữ 1 giờ trong bộ nhớ, ≤3MB/tệp, ≤16 tệp chờ/người nhận; chỉ **người nhận** `GET /v1/blob/<id>` được, lấy một lần là xoá. Tin `hoi` mang `anh: [id…]` (≤4) trong hộp đã ký. Thử: `build/relay-blob-smoke.mjs` |
